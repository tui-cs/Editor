...
{
    /// <summary>Asserts that the rendered driver contents contain <paramref name="expected"/> as a substring (whitespace insensitive at line ends).</summary>
    public static void ContentsContains (IDriver driver, string expected)
    {
        ArgumentNullException.ThrowIfNull (driver);
        ArgumentNullException.ThrowIfNull (expected);

        string actual = driver.ToString () ?? string.Empty;
