...
| `Highlighting/HighlightingColor.cs` | **Fork addition (syntax-theme, `specs/syntax-theme/spec.md`).** Added `VisualRole? Role` — the Terminal.Gui scheme role this color resolves to (populated at load; consumed by `Rendering/HighlightingColorizer`). Not an upstream concept; AvaloniaEdit bakes colors directly. `MergeWith` carries `Role` over. Excluded from `Equals`/`GetHashCode` (derived from `Name`). |
